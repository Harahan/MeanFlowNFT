...
def _filter_lora_state_dict(state_dict: dict) -> dict:
    ...
    Keeps:
      - LoRA adapter weights (``lora_A`` / ``lora_B`` keys), produced by
        ``peft.add_adapter`` for the LoRA-enabled AnyFlow trainers.
      - AnyFlow flow-map ``delta_embedder.*`` weights, which are NOT in a
        LoRA adapter but ARE trainable (full-FT inside
        ``SD3FlowMapTimeTextEmbed``).
        Without this exception, AnyFlow LoRA save would drop the r-conditioning
        weights entirely and Stage 2 resume / inference would silently revert
        the model to ``delta_embedder == timestep_embedder.deepcopy()`` (the
        wrapper's init state) — i.e. all Stage 1 r-conditioning would vanish.
