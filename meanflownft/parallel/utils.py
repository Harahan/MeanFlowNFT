...
initialization. Adapted from ViGen-DiT's parallel utilities with video-specific
logic removed.
...
def setup_distributed() -> None:
    ...
    # collectives idle far longer than that -- e.g. the standalone VBench eval
    # generates thousands of videos (skew across ranks at the post-gen barrier) and
    # then scores them on rank 0 while the other ranks wait at a barrier. Use a
...
def get_transformer_wrap_policy(transformer_block_cls: type | set[type] | tuple[type, ...]) -> Callable:
    ...
    Args:
        transformer_block_cls: The class or classes of transformer blocks to
            wrap at (for this branch, Wan transformer blocks).
