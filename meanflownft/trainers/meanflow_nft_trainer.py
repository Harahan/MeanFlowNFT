"""
AnyFlow + DiffusionNFT Trainer for MeanFlowNFT.

Combines the AnyFlow flow-map (``u_theta(x_t, r, t)``) network parameterization
...
network remains AnyFlow-parameterized (preserving few-step generation).

The central difference (no-grad) is the stop-gradient approximation of
the analytic ``dF/dt`` in the derivation, mirroring AnyFlow pretrain's
...
``MeanFlowNFTRLConfig.cd_velocity_source``:
  ...
  - "noise_minus_x0" (default): ``v_dir = eps - x_0`` (original AnyFlow).
...
The rollout uses AnyFlow's eval-time N-step Euler flow-map sampler
...
class MeanFlowNFTTrainer(NFTTrainer):
    """DiffusionNFT for AnyFlow-style flow-map (average velocity) models."""

    def __init__(self, config: MeanFlowNFTConfig):
        super().__init__(config)
        # Schedulers / dynamic-shifting state — initialised in setup_models.
        self.flowmap_scheduler: Optional[FlowMapScheduler] = None
        self._use_dynamic_shifting: bool = False
        self._image_seq_len: int = 0
    ...
    # ------------------------------------------------------------------
    # Config accessor — point at the AnyFlow-specific NFT block.
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Override setup_models to compute latent_size BEFORE _pre_setup_lora
    # (because flow-map scheduler init needs latent_size for image_seq_len).
    # ------------------------------------------------------------------

    def _get_last_rollout_timestep_pairs(self) -> tuple[torch.Tensor, torch.Tensor]:
        """Return ``(t_arr, r_arr)`` from the AnyFlow flow-map rollout.
    ...
    def _central_diff_dudt(
        ...
        """Estimate the path derivative with boundary-aware finite differences."""
        T = float(self.scheduler.config.num_train_timesteps)
        eps_cd = float(self._nft_cfg().central_diff_epsilon)
        t_plus = (t_raw + eps_cd).clamp(max=T)
        # Holding r fixed requires every perturbed endpoint to satisfy r <= t.
        t_minus = torch.maximum((t_raw - eps_cd).clamp(min=0.0), r_raw)
        plus_delta = t_plus - t_raw
        minus_delta = t_raw - t_minus
        view_shape = (x_t.shape[0],) + (1,) * (x_t.ndim - 1)
        z_plus = x_t + v_dir * (plus_delta / T).view(view_shape).to(x_t.dtype)
        z_minus = x_t - v_dir * (minus_delta / T).view(view_shape).to(x_t.dtype)
        ...
        denominator = (t_plus - t_minus).view(view_shape)
        valid = denominator > 0
        derivative = (f_plus - f_minus) / denominator.clamp_min(1e-6).to(
            f_plus.dtype
        )
        return torch.where(valid, derivative, torch.zeros_like(derivative))
    ...
    def _compute_dF_dt_for_model(
        ...
        *,
        ...
        ``v_dir`` is selected by ``meanflow_nft.cd_velocity_source``:
          ...
          - ``"noise_minus_x0"``: use the exact sampled noise and
            ``v_dir = eps - x_0`` (original AnyFlow; model-independent).
        ...
        if cfg.cd_velocity_source == "noise_minus_x0":
            if noise is None:
                raise ValueError(
                    "MeanFlowNFT central difference requires the exact sampled noise."
                )
            v_dir = noise.to(dtype=x_0.dtype) - x_0
    ...
    def _pre_compute_v_hook(
        ...
        *,
        ...
        if r_for_model is None:
            # Vanilla NFT (no flow-map (t, r) pair) — nothing to share.
            return
    ...
    def _compute_u(
        ...
        *,
        ...
        """Leading AnyFlow flow-map output u(x_t, r, t)."""
    ...
    def _compute_v(
        ...
        *,
        ...
        # AnyFlow's training-time (t, r) sampler continues to draw r/t from
        ...
        # speedup for pure-diffusion configurations.
        #
        ...
        # AnyFlow's ``target = v_target - (t-r)*dF/dt`` form. The check is
        ...
        # Matches AnyFlow pretrain's PDE-residual target form
        # ``target = v_target - (t - r) * dF/dt`` exactly.
        diff = (t - r_raw).view(x_t.shape[0], *([1] * (x_t.ndim - 1)))
        return u + diff.to(u.dtype) * dF_dt.to(u.dtype)

    # ==================================================================
    # Training-time (t, r) sampling — AnyFlow-flavored, decoupled from rollout
    #
    ...
    def _num_training_timesteps(self, n_ts_total: int) -> int:
        """AnyFlow: decoupled from rollout N — use the config knob.

        Falling back to :meth:`NFTTrainer._num_training_timesteps` (and thus to
        the rollout-aligned (t, r) schedule) when
        ``num_training_timesteps_per_sample <= 0`` lets users A/B-test the new
        sampling against the old behavior without code changes.
        """
        n = int(self._nft_cfg().num_training_timesteps_per_sample)
        if n <= 0 and self._nft_cfg().nft_velocity_mode == "direct_u":
            n = int(self._nft_cfg().sampling_num_steps or n_ts_total)
        if n <= 0:
            return super()._num_training_timesteps(n_ts_total)
    ...
    def _prepare_inner_epoch_tr_state(self, **_kwargs: Any) -> dict[str, Any]:
        """AnyFlow draws fresh per iter — no pre-shuffle needed."""
        return {}
    ...
    def _sample_training_tr_three_mode(
        ...
        *,
        ...
        """Draw one (t, r) batch per AnyFlow three-mode partition + shift+clamp.
        ...
        3. Global-index partition (``global_idx = rank * batch_size + b``):
            ...
            - rest                                           : keep ``r01``.
        4. Apply the checkpoint's static shift to both endpoints and scale by T.
        ...
        # Sample continuously in fp32; bf16 quantizes the (t, r) distribution.
        t1 = torch.rand(
            batch_size, dtype=torch.float32, device=device, generator=gen
        )
        t2 = torch.rand(
            batch_size, dtype=torch.float32, device=device, generator=gen
        )
        ...
        # Shift both endpoints onto the same static Wan timestep grid.
