"""
DiffusionNFT Trainer for MeanFlowNFT.

Vanilla DiffusionNFT (arXiv 2509.16117) for RL fine-tuning of generators
previously distilled with DMD / CDM / etc. Operates in instantaneous-velocity
(v) space:
...
Per train_step:
    ...
    2. Sampling phase (no_grad): rollout x_0 from `old_model` (or
       `generator_ema` if configured) with the active denoising scheduler
       (consistency / euler / ddim, picked via `dmd.generator_scheduler`).
...
Inherits :class:`BaseTrainer` directly. Model loading mirrors
:class:`DMDTrainer.setup_models` but drops the fake-score / teacher
machinery — NFT only needs the generator + a frozen reference (old_model).
"""
...
class NFTTrainer(BaseTrainer):
    """Vanilla DiffusionNFT trainer for v-prediction generators."""

    def __init__(self, config: MeanFlowNFTConfig):
        super().__init__(config)
        self.model_type = str(self.config.model.model_type).lower()
        self._is_wan = self.model_type == "wan"
        if not self._is_wan:
            raise ValueError(
                f"The wan branch only supports model_type=\'wan\'; got {self.model_type!r}."
            )
        ...
        self._reward_scorer: Optional[Any] = None
        self._stat_tracker: Optional[PerPromptStatTracker] = None

        # Cached unconditional embeds (for sampling guidance > 1 only).
        self._uncond_embeds: Optional[tuple[torch.Tensor, torch.Tensor]] = None
        # Cached denoising sigma schedules for rollout.
        self._denoising_sigmas_cache: dict[int, list[float]] = {}

        # Epoch-based outer loop state (mirrors FlowGRPOTrainer / DiffusionNFT).
    ...
    # ==================================================================
    # Model setup (mirrors DMDTrainer pattern, minus fake_score/teacher)
    # ==================================================================
    ...
    # ==================================================================
    # Hook accessors (subclasses override _nft_cfg() to point elsewhere)
    # ==================================================================

    def _nft_cfg(self):
        """Return the active NFT config block. Subclasses point at their own."""
    ...
    def _pre_compute_v_hook(
        ...
        *,
        ...
        noise: torch.Tensor,                  # noqa: ARG002
    ) -> None:
    ...
    def _compute_advantages_weighted(
        ...
        GRPO trainer's ``weight_advantages`` path. Returns this rank's slice.
    ...
    def _nft_loss(
        ...
        # 2. Velocity predictions. ``x_0`` and ``r_raw`` are passed through
        # for the AnyFlow override (which needs them for central-diff V_theta);
        # the vanilla _compute_v ignores them.
        ...
        try:
            v_theta = self._compute_v(
                ...
                with_grad=True, x_0=x_0, r_raw=r_for_model, noise=eps,
            )
            v_old = self._compute_v(
                self.old_model, x_t, t_for_model, prompt_embeds, pooled_embeds,
                with_grad=False, x_0=x_0, r_raw=r_for_model, noise=eps,
            ).detach()
            v_ref = None
    ...
    def _current_ema_decay(self) -> float:
        ...
        Mirrors ``DiffusionNFT/flow_grpo/ema.py::EMAModuleWrapper.get_current_decay``:
    ...
    def _one_sampling_batch(self, prompts: list[str]) -> dict[str, Any]:
        ...
        num_steps = cfg.sampling_num_steps or len(self.config.dmd.denoising_step_list)
        sampler_model = (
        ...
        # that pattern so AnyFlow's flow-map central diff can use the exact
        # (t, r) pair the rollout used at inference (vanilla NFT only needs
        # ``t`` and ignores ``r``).
    ...
    def _get_last_rollout_timestep_pairs(self) -> tuple[torch.Tensor, torch.Tensor]:
        """Return ``(t_arr, r_arr)`` for the last rollout, both shape ``[N]``.

        For vanilla v-prediction NFT, ``r_arr`` is unused by the loss but
        kept for storage uniformity (matches DiffusionNFT's
        ``next_timesteps`` convention; default = the next ``t`` value or 0
        at the boundary, mirroring
        ``torch.cat([timesteps[1:], torch.zeros(1)])``).

        AnyFlow overrides this to return the actual ``(t_i, t_{i+1})``
        flow-map jump pairs from ``flowmap_scheduler.timesteps``.
        """
        ts = self.x0_scheduler.timesteps.detach().to(dtype=torch.float32)
        zeros = torch.zeros((1,), device=ts.device, dtype=ts.dtype)
        return ts, torch.cat([ts[1:], zeros], dim=0)
    ...
    # ==================================================================
    # Training-time (t, r) sampling hooks (overridden by MeanFlowNFTTrainer)
    # ==================================================================

    def _num_training_timesteps(self, n_ts_total: int) -> int:
        """How many (t, r) iterations per inner-batch chunk.

        Vanilla NFT: ``floor(N_rollout * timestep_fraction)``, matching
        DiffusionNFT's ``num_train_timesteps`` derivation. AnyFlow overrides
        this to a config-knob count fully decoupled from the rollout schedule.
        """
        cfg = self._nft_cfg()
        n_train_ts = max(1, int(n_ts_total * float(cfg.timestep_fraction)))
        return min(n_train_ts, max(1, n_ts_total))
    ...
    def _prepare_inner_epoch_tr_state(
        ...
        *,
        ...
        n_ts_total: int,
        n_train_ts: int,    # noqa: ARG002 - used by AnyFlow override
        device: torch.device,
    ) -> dict[str, Any]:
        """Per-inner-epoch precomputation feeding :meth:`_draw_training_tr`.

        Vanilla NFT: do the per-sample timestep shuffle once per inner epoch
        (DiffusionNFT/scripts/train_nft_sd3.py:873-879) and return the
        already-shuffled (t, r) arrays. ``_draw_training_tr`` then just slices
        by ``[chunk_start:chunk_end, j_idx]``.

        AnyFlow override returns an empty dict and draws fresh (t, r) per
        iteration via the three-mode partition.
        """
        sts = timesteps_all[sample_perm]
        snts = next_timesteps_all[sample_perm]
        n_local = sts.shape[0]
        perms_time = torch.stack(
            [torch.randperm(n_ts_total, device=device) for _ in range(n_local)]
        )
        row_idx = torch.arange(n_local, device=device)[:, None]
        return {
            "shuffled_t": sts[row_idx, perms_time],   # [n_local, n_ts_total]
            "shuffled_r": snts[row_idx, perms_time],  # [n_local, n_ts_total]
        }
    ...
    def _draw_training_tr(
        ...
        *,
        ...
        chunk_end: int,
        j_idx: int,
        inner_epoch_idx: int,   # noqa: ARG002 - used by AnyFlow override
        device: torch.device,   # noqa: ARG002 - used by AnyFlow override
        dtype: torch.dtype,     # noqa: ARG002 - used by AnyFlow override
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """Return ``(t_raw, r_raw)`` for this inner-batch iteration.

        Vanilla NFT: pop the ``j_idx``-th column of the per-sample-shuffled
        rollout arrays. AnyFlow override: independent three-mode partition.
        """
        return (
            state["shuffled_t"][chunk_start:chunk_end, j_idx],
            state["shuffled_r"][chunk_start:chunk_end, j_idx],
        )
    ...
    def _nft_train_phase(
        ...
        - ``num_train_timesteps`` is decided by :meth:`_num_training_timesteps`
          (vanilla: ``floor(N_rollout * timestep_fraction)``; AnyFlow override:
          config-knob count decoupled from rollout NFE).
        - The (t, r) draw is dispatched through :meth:`_draw_training_tr`
          (vanilla: rollout-aligned shuffled lookup; AnyFlow override:
          fresh three-mode partition per iteration).
        """
        cfg = self._nft_cfg()
        device = torch.device("cuda")
        ...
        # Release the caching-allocator blocks left over from the sampling phase
        # (few-step rollout + VAE decode + reward VLMs all ran on GPU and freed
        # their tensors, but the freed blocks stay reserved and fragmented). The
        # train phase's backward needs large contiguous blocks for FSDP
        # all-gather / reduce buffers; reclaiming once per epoch here avoids the
        # OOM without touching per-step compute (called once per NFT epoch).
        torch.cuda.empty_cache()
        ...
        for inner_epoch_idx in range(n_inner_epochs):
            ...
            # Per-sample (t, r) state: vanilla pre-shuffles rollout arrays;
            # AnyFlow override returns an empty dict and draws fresh per iter.
            ...
            for start in range(0, n_local, inner_bs):
                end = min(start + inner_bs, n_local)
                x0_b = sx0[start:end]
                pe_b = spe[start:end]
                pp_b = spp[start:end]
    ...
    def _run_nft_epoch(self) -> dict[str, dict[str, float]]:
        ...
        (aligned with :class:`FlowGRPOTrainer`):
        ...
        # Optional sampling-phase preview media. Image generators log images;
        # video subclasses (Wan) override ``_log_sampling_preview`` to log mp4.
        self._log_sampling_preview(preview)

        return metrics

    def _log_sampling_preview(self, preview) -> None:
        """Log sampling-phase rollout previews to wandb (images).

        ``preview`` is ``(media_tensor, prompts)`` where ``media_tensor`` is the
        decoded samples from the first sampling batch (``[B, C, H, W]`` for image
        generators). No-op unless ``log_sample_images > 0``. Video subclasses
        override this to log ``[B, F, 3, H, W]`` clips as mp4.
        """
        cfg = self._nft_cfg()
        if (
    ...
    def _train_loop(self) -> None:
        """Epoch-based outer loop. Mirrors :class:`FlowGRPOTrainer` and the
        DiffusionNFT training script's `for epoch in range(num_epochs)` shape.
        """
        cfg = self._nft_cfg()
    ...
    # ==================================================================
    # Init helper (reused from DMDTrainer pattern)
    # ==================================================================
    ...
    # ==================================================================
    # Grad-norm helpers (mirror DMDTrainer)
    # ==================================================================
