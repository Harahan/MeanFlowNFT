...
Class hierarchy:
    BaseTrainer
      ...
      │     └── PhasedDMDTrainer (one-step-one-phase sub-interval objective)
      └── FlowGRPOTrainer       (online RL via SDE sampling + PPO)
            └── FlowGRPODMDTrainer (+ last-step DMD/regression aux loss)
...
class BaseTrainer(ABC):
    ...
    def evaluate(self) -> None:
        """Load a checkpoint and run reward/VBench evaluation once."""
        if not self.config.train.resume_from:
            raise ValueError(
                "Evaluation-only mode requires train.resume_from=<checkpoint-dir>."
            )
    ...
    def _setup_environment(self) -> None:
        ...
        # Append a timestamp for run isolation.
        if is_main_process():
    ...
    def _save_final_transformer(self) -> None:
        ...
        For full-weight training:
            Saves in Diffusers format using the configured transformer class.
    ...
    def _load_eval_prompts_and_metadata(
        ...
        if use_dataset:
            # Load from the configured dataset directory.
        ...
        for dataset_name, reward_names in dataset_to_rewards.items():
            ...
            for reward_name in reward_names:
                ...
                if reward_failed_local:
                    ...
                    # Subclasses can defer this for multi-pass evaluation reuse.
