...
except Exception:  # flex_attention only needed for the unused causal FAR path
...
# Public alias for MeanFlowNFT. This vendored model is the bidirectional
# AnyFlow-Wan flow-map transformer (two-time (t, r) conditioning baked in
# via setup_flowmap_model()). The causal FAR code paths are retained for
# bit-exact checkpoint loading but are unused by wan_anyflow_nft.
WanAnyFlowTransformer3DModel = FAR_Wan_Transformer3DModel
