...
- ``apply_shift`` uses the static rational shift from the AnyFlow-Wan checkpoint.
...
class FlowMapScheduler(SchedulerMixin, ConfigMixin):
    ...
    Args:
        ...
        shift: Static AnyFlow-Wan shift parameter applied to ``set_timesteps``
            output. ``shift = 1.0`` means no shift; ``shift = 5.0`` matches the released AnyFlow-Wan policy.
