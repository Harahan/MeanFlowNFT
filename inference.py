"""Command-line entry point for Wan2.1 inference."""
...
from meanflownft.inference import InferenceConfig, WanInference
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run normal Wan, AnyFlow, or MeanFlowNFT inference."
    )
    parser.add_argument("config", help="Path to the Wan inference YAML")
    parser.add_argument(
        "--override",
        nargs="+",
        action="append",
...
def main() -> None:
    ...
    config.validate()
    engine = WanInference(config)
    try:
        engine.setup()
        engine.run()
    except BaseException:
        logger.exception("Wan2.1 inference failed")
        raise
    finally:
        engine.close()
